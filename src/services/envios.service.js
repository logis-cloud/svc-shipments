...
async function crear(data){
 ...
 const principal=direcciones.find(d=>d.es_principal===true)||direcciones[0], ahora=new Date();
 const envio=await Envio.create({codigoSeguimiento:generarCodigoSeguimiento(),clienteId:data.clienteId,pedidoId:data.pedidoId,estado:"CREADO",direccionEntrega:{calle:principal.calle,distrito:principal.distrito,ciudad:principal.ciudad,codigoPostal:principal.codigo_postal,referencia:principal.referencia},items:data.items,fechaCreacion:ahora,fechaActualizacion:ahora,transportista:data.transportista,placaVehiculo:data.placaVehiculo});
